...
WaitNode* 
WaitingTaskList::createNode(std::shared_ptr<WaitingTask> iTask)
{
   ...
   returnValue->m_task = std::move(iTask);
...
WaitingTaskList::add(WaitingTaskHolder iTask) {
   ...
   if(!m_waiting) {
      ...
      WaitNode* newHead = createNode(std::move(t));
