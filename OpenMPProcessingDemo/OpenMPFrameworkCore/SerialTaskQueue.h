...
namespace demo {
   ...
   public:
      ...
      */
       ...
       */
      template<typename T>
      demo::TaskBase* pushAndGetNextTaskToRun(const T& iAction);
   ...
   private:
      ...
      class TaskBase : public demo::TaskBase {
         friend class SerialTaskQueue;
         TaskBase(): m_queue(0) {}
         
      protected:
         demo::TaskBase* finishedTask();
      private:
      ...
      demo::TaskBase* pushAndGetNextTask(TaskBase*);
      demo::TaskBase* finishedTask();
   ...
   void SerialTaskQueue::push(const T& iAction) {
      QueuedTask<T>* pTask{ new QueuedTask<T>{iAction} };
      pTask->setQueue(this);
      pushTask(pTask);
   }
   ...
   template<typename T>
   demo::TaskBase* SerialTaskQueue::pushAndGetNextTaskToRun(const T& iAction) {
      QueuedTask<T>* pTask{ new QueuedTask<T>{iAction} };
      pTask->setQueue(this);
   ...
   template <typename T>
   void
   SerialTaskQueue::QueuedTask<T>::doSpawn() {
      try {
         this->m_action();
      } catch(...) {}
      if(auto next =this->finishedTask()) {
        demo::TaskBase::spawn(next);
      }
   }
