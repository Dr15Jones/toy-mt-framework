...
namespace demo {
  class EventTimesBusyWaitParallelForPassFilter : public EventTimesPassFilterBase {
  public:
    explicit EventTimesBusyWaitParallelForPassFilter(const boost::property_tree::ptree& iConfig);
  private:
    void wait(double iSeconds) override;
    const unsigned int m_nRanges;
  };
  
  EventTimesBusyWaitParallelForPassFilter::EventTimesBusyWaitParallelForPassFilter(const boost::property_tree::ptree& iConfig):
  ...
  void
  EventTimesBusyWaitParallelForPassFilter::wait(double iSeconds){
...
REGISTER_FILTER(demo::EventTimesBusyWaitParallelForPassFilter);
